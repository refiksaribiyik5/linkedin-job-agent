...
- **Bağımlılıklar:** M0.1.
- **Beklenen Sonuç:** Her varlık, PRD 15.x'teki alan listesiyle birebir örtüşen, doğrulanabilir bir model olarak var olur; hiçbir model dış kütüphaneye (DB, HTTP) bağımlı değildir. `FilterResult`, ayrıca yalnızca departman filtresinin doldurduğu (diğer filtrelerde `None` kalan) opsiyonel bir `matched_cluster: str | None` alanı taşır — M9.3'ün implementasyonu sırasında bulunan bir boşluğun düzeltmesidir: `filtering/department_filter.py`'nin (M6.4) zaten hesapladığı eşleşen departman kümesi adı, yalnızca serbest metin `reason` alanına gömülüyordu; `EvaluatedJob.department_cluster`'ı (FR-11, Must-have departman gruplama) doldurmak için Orchestrator'ın metin ayrıştırması yapmasını veya `department_filter.py`'nin dahili mantığını yeniden uygulamasını gerektirirdi — ikisi de kaçınılması istenen desenlerdir. `RunLog`, ayrıca `status == Partial` iken zorunlu, aksi durumda kullanılmayan bir `partial_reason: str | None` alanı taşır — M9.4'ün (Hata Sınıflandırma + Retry) implementasyonu sırasında bulunan bir boşluğun düzeltmesidir: Section 21'in devre kesici (circuit-breaker-lite) mekanizması bir çalıştırmayı 'Partial' işaretlediğinde, bu kararın gerekçesini taşıyacak ayrı bir alan yoktur. Mevcut `error_detail` alanı bunun için kullanılmaz: PRD 15.6 bu alanı açıkça 'hata bilgisi' olarak tanımlar ve yalnızca `Failed` durumunda zorunludur (bkz. `_failed_runs_are_never_silent` doğrulayıcısı); Partial bir çalıştırma bir hata değildir — Section 15.7 Partial'ı Success ile aynı şekilde her zaman bir rapor üreten, tam teşekküllü bir sonuç olarak tanımlar. `error_detail`'in Partial için yeniden kullanılması bu ayrımı bulanıklaştırırdı. Bu, `collection_capped`'in (PRD 15.6'nın alan listesinde açıkça yer almayan ama FR-21'in zorunlu kıldığı bir alan) eklendiği aynı örüntünün tekrarıdır.
- **Tamamlanma Doğrulaması:** Birim testleri her modeli geçerli veriyle örnekler ve geçersiz veriyle (örn. skor aralığı dışı bir AI Match Score) `ValidationError` fırlatıldığını doğrular. `FilterResult.matched_cluster`'ın varsayılan olarak `None` olduğu ve açıkça verildiğinde doğru şekilde taşındığı ayrıca doğrulanır. `partial_reason`'ın `status == Partial` iken zorunlu, diğer tüm durumlarda kullanılmadığı (mevcut `error_detail`/`Failed` doğrulayıcısının simetriği) ayrıca doğrulanır.
- **Tahmini Süre:** 3–5 saat.

### M1.2 — Veritabanı Şeması v1
- **Amaç:** TDD Section 15'teki (v1.1 düzeltmeleri dahil) tüm tabloları somutlaştırmak: `accounts` (+ `next_run_at`), `user_profiles`, `account_config_profiles` (+ `target_criteria`), `linkedin_sessions`, `evaluated_jobs`, `reports`, `run_logs` (+ `partial_reason`, M9.4 düzeltmesi), `run_locks` (+ `lock_expires_at`), `companies`, `company_scores` (+ `rubric_version`), `job_postings`.
...
- **Beklenen Sonuç:** `alembic upgrade head` çalıştırıldığında TDD v1.1'deki her tablo ve sütun (5 mimari düzeltme dahil) eksiksiz oluşur. `run_logs.partial_reason` alanı (bkz. M1.1 düzeltmesi), ayrı bir `migrations/versions/0002_run_logs_partial_reason.py` migration'ı ile eklenir — zaten uygulanmış olan `0001_initial_schema.py` geriye dönük değiştirilmez.
...
- **Bağımlılıklar:** M1.1.
- **Beklenen Sonuç:** Geçerli bir config kabul edilir; toplamı %100 olmayan ağırlıklar veya tanımsız bir departman referansı reddedilir ve anlaşılır bir hata mesajı üretir. `Thresholds`, ayrıca yapılandırılabilir bir `department_confidence_tolerance: float` (0–1 aralığında) alanını taşır — M9.3'ün tasarım incelemesinde bulunan bir boşluğun (Gap B) düzeltmesidir: `filtering/pipeline.py`'nin (M6.5, değiştirilmemiş) Department borderline mantığı bu değeri zorunlu bir parametre olarak alır, ancak `borderline_band_width` (yalnızca 0–100 puanlık AI Match Score ölçeğinde tanımlı) M6.5'in kendi onaylanmış kararıyla buraya doğrudan uygulanamaz. `Thresholds`, ayrıca M9.4'ün (Hata Sınıflandırma + Retry) ihtiyaç duyduğu beş yapılandırılabilir alanı taşır: `linkedin_retry_attempts: int` (LinkedIn sayfa/sorgu başına azami deneme sayısı), `llm_retry_attempts: int` (LLM çağrısı başına azami deneme sayısı), `retry_base_delay_ms: int` ve `retry_max_delay_ms: int` (üstel geri çekilmenin başlangıç ve üst sınır gecikmeleri, milisaniye — Section 21), `linkedin_consecutive_failure_threshold: int` (devre kesicinin tetiklenmesi için gereken ardışık LinkedIn hata sayısı — Section 21 circuit-breaker-lite). Bu, M9.4'ün implementasyonu sırasında bulunan bir boşluğun düzeltmesidir: Section 21 bu parametreler için yalnızca örnek değerler ("örn. 3") verir; NFR-6/FR-13 gereği hiçbiri kod içine sabitlenemez.
...
- **Oluşturulacak Dosyalar:** `docker-compose.yml` (tamamlanmış), `Dockerfile` (tamamlanmış), `main.py`, `bootstrap.py`, yedekleme script'i.
- **Bileşenler:** Dağıtım altyapısı.
- **Bağımlılıklar:** Yukarıdaki tüm milestone'lar.
- **Beklenen Sonuç:** `docker compose up` tüm sistemi çalıştırır. `main.py`, Faz 9'un (M9.6/M9.7) tasarım incelemesinde bulunan bir sahiplik boşluğunun düzeltmesidir: TDD Section 5/9, `main.py`'yi sürecin giriş noktası (scheduler döngüsü) olarak tanımlar, ama hiçbir milestone onun oluşturulmasını kendi dosya listesine almamıştı — M9.6 kasıtlı olarak yalnızca zamanlama mekanizmasını (`ports/scheduler_port.py`, `adapters/scheduling/apscheduler_adapter.py`), M9.7 kasıtlı olarak yalnızca CLI'yi (`cli.py`) kapsamına aldı; ikisi de doğru kararlardı, ama M10.1'in kendi "Beklenen Sonuç"u ("docker compose up tüm sistemi çalıştırır", gözetimsiz bir zamanlanmış çalıştırma dahil) bu dosya olmadan karşılanamaz. `main.py` YALNIZCA TDD Section 9'un tanımladığı uzun-ömürlü composition root'tur: `SchedulerPort`/`APSchedulerAdapter`'i (M9.6) ve Orchestrator'ı (M9.3) birbirine bağlar — hiçbir yeni iş mantığı İÇERMEZ, ikinci bir mimari İCAT ETMEZ. Bağımlılık kurma mantığı (`build_dependencies()`/`run_account()`, M9.7'de `cli.py` içinde `_build_dependencies()` olarak inşa edilmişti) `bootstrap.py`'ye taşınır — bağımsız incelemede bulunan bir tasarım bulgusunun düzeltmesidir: `main.py`'nin `cli.py`'den doğrudan import etmesi, TDD Section 5'in `main.py`/`cli.py`'yi EŞIT SEVIYEDE, birbirinden BAĞIMSIZ giriş noktaları olarak tanımlayan dosya ağacıyla çelişen yanal (sideways) bir bağımlılık yaratırdı. `cli.py` YALNIZCA CLI sunumundan (argparse, çıkış kodları, stdout/stderr) sorumlu kalır; `main.py` YALNIZCA uzun-ömürlü zamanlayıcı sürecinden sorumlu kalır; ikisi de `bootstrap.py`'ye bağımlıdır, birbirlerine DEĞİL. `run_account()`, `bootstrap.py`'ye taşınırken `trigger_type: TriggerType` parametresini ACIKCA alacak sekilde genellenir (M9.7'deki hali `TriggerType.MANUAL`'i sabit kodluyordu) - `main.py`'nin zamanlanmış calistirmalari `TriggerType.SCHEDULED` ile dogru isaretlemesi icin gereklidir; `cli.py`'nin kendi cagri noktasi `TriggerType.MANUAL`'i acikca gecirecek sekilde guncellenir. `_report_run_result()` (stdout/stderr formatlama, cikis kodu) `cli.py`'de KALIR - bu CLI'ye ozeldir, `main.py` sureklilik gerektirdigi icin (her calistirmadan sonra cikmaz) ayni sekle ihtiyac duymaz; `main.py`'nin sonuc gorunurlugu zaten `orchestrator.run()` icindeki StructuredLogger cagrilari araciligiyla saglanir.
...
- **Bağımlılıklar:** Yok.
- **Beklenen Sonuç:** `TriggerType.SCHEDULED` bir çalıştırma `SessionInvalidError` ile başarısız olduğunda, tek native bir sinyal tetiklenir — yeni bir kuyruk/webhook/e-posta altyapısı EKLENMEZ; bu, PRD Section 18'in Faz 2 kapsamındaki "bildirimler" özelliğiyle KARIŞTIRILMAMALIDIR (bu madde ona bir alt-yapı İNŞA ETMEZ, yalnızca tek, sabit-kodlu bir yerel sinyal ekler). **Uygulama notu (implementasyon sırasında bulunan bir kısıtlamanın sonucu):** `main.py` gerçek dağıtımda Docker/Linux konteyneri içinde çalışır (bkz. Dockerfile `CMD`) — macOS'a özgü bir bildirim mekanizması (örn. `osascript`) konteynerden doğrudan tetiklenemez ve host'a sızmaz. Bunun yerine, zaten bind-mount edilmiş `reports/` dizinine (kullanıcının raporları okumak için ZATEN baktığı tek yer) `NEEDS_LOGIN.txt` adlı, açıkça adlandırılmış bir işaret dosyası yazılır; bir sonraki başarılı zamanlanmış çalıştırma bu dosyayı temizler, böylece dosyanın varlığı her zaman "şu an çözülmemiş" anlamına gelir.
